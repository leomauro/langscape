...
class Code(object):
    ...
    def from_code(cls, co):
        ...
        while i < n:
            ...
            if i in labels:
                code.append((labels[i], None,i))
            if i in linestarts:
                code.append((SetLineno, linestarts[i],i))
            i += 1
            if op in hascode:
                lastop, lastarg, i = code[-1]
                if lastop != LOAD_CONST:
                    raise ValueError, \
                          "%s should be preceded by LOAD_CONST code" % op
                code[-1] = (LOAD_CONST, Code.from_code(lastarg), i)
            if op not in hasarg:
                code.append((op, None,i))
            else:
                ...
                if op == opcode.EXTENDED_ARG:
                    ...
                    code.append((op, co.co_consts[arg],i))
                elif op in hasname:
                    code.append((op, co.co_names[arg],i))
                elif op in hasjabs:
                    code.append((op, labels[arg],i))
                elif op in hasjrel:
                    code.append((op, labels[i + arg],i))
                elif op in haslocal:
                    code.append((op, co.co_varnames[arg],i))
                elif op in hascompare:
                    code.append((op, cmp_op[arg],i))
                elif op in hasfree:
                    code.append((op, cellfree[arg],i))
                else:
                    code.append((op, arg,i))
    ...
    def __eq__(self, other):
        ...
        # not equal.
        labelmapping = {}
        for (op1, arg1, _), (op2, arg2, _) in itertools.izip(self.code, other.code):
...
def printallcodes(codeobj, indent = 0):
    for op, val, i in codeobj.code:
        print " "*indent, (op, val, i)
        if isinstance(val, Code):
            printallcodes(val, indent+4)
